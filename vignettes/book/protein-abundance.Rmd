...
We thus remove cells that have unusually low numbers of detected ADTs using the usual outlier-based approach.
...
ab.discard <- isOutlier(df.ab$detected, log=TRUE, type="lower", min_diff=1)
summary(ab.discard)
...
To elaborate on the `isOutlier()` call above:
we set `min_diff=1` to specify that the threshold should be defined at least one log~2~-unit below the median.
This ensures that cells will not be removed if they contain non-zero counts for more than half of the median number of detected features.
We do not rely only on the default behavior of `isOutlier()` as the MAD is often zero in deeply sequenced datasets (i.e., where most cells contain non-zero counts for all ADTs) and filtering would discard useful cells that detect _almost_ all of the ADTs.

Some experiments will include isotype control (IgG) antibodies that have similar properties to a primary antibody but lack a specific target in the cell, thus providing a measure of non-specific binding.
Assuming that the magnitude of non-specific binding should be roughly constant across cells, we can define low-quality cells as those with unusually low coverage of the control ADTs, presumably due to failed capture or sequencing.
This again uses our outlier-based approach; hard thresholds are more difficult to specify due to experiment-by-experiment variation in the expected coverage of ADTs. 
...
hist(log1p(df.ab2$subsets_controls_sum), col='grey', 
    main="", xlab="Log-coverage of controls")
abline(v=log1p(attr(ab.discard, "thresholds")["lower"]), col="red", lty=2)
```
