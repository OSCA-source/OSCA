...
So ambient removal can generally be omitted, though we will describe it here regardless as it can be useful in specific situations.

Ambient removal is difficult as we need to know how much contamination is present in each cell.
This usually requires some prior biological knowledge about genes that should not be expressed in the dataset (e.g., mitochondrial genes in single-nuclei datasets, see Section \@ref(nuclei-ambient-tricks)) or genes with mutually exclusive expression profiles [@young2018soupx], where those same genes must also be highly abundant in the ambient solution.
The actual removal of the ambient contribution also must be done in a manner that respects the mean-variance relationship of the count data.
Thus, rather than attempting to remove contamination from individual cells, a more sensible approach is to remove it from the cluster-level profiles and the propagate those changes to their constituent cells.

We demonstrate this process using the `removeAmbience()` function on our PBMC dataset.
This function aggregates counts in each cluster to obtain an average profile, estimates the contamination proportion in each cluster with `maximumAmbience()` (Section \@ref(ambient-problems)), subtracts the contamination from the cluster-level average, and then performs quantile-quantile mapping of each individual cell's counts from the old average to the new subtracted average.
This has the useful property of not requiring any prior knowledge of control or mutually exclusive expression profiles, and it provides a statistically justifiable method for subtraction that preserves the mean-variance relationship.
(That said, if feature information is available, we can specify it in the `features=` argument to obtain results more similar to those from the `r CRANpkg("SoupX")` package.)

```{r, results='asis', echo=FALSE}
extractCached("tenx-unfiltered-pbmc4k.Rmd", "clustering", c("e.out"))
extractCached("tenx-unfiltered-pbmc4k.Rmd", "clustering", c("sce.pbmc"))
```
